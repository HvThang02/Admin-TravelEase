...
export default function HotelFacility() {
  ...
  const [hotelFacilityData, setHotelFacilityData] = useState<DataType[]>([]);
  const [hotelFacilityInput, setHotelFacilityInput] = useState("");

  useEffect(() => {
    const fetchFacilityType = async () => {
      try {
        const response = await axios.get(`${api}/hotel-facility`);
        const data = response.data;
        setHotelFacilityData(data);
      } catch (error) {
  ...
  const addNewHotelFacility = async () => {
    try {
      const response = await axios.post<DataType>(`${api}/hotel-facility`, {
        hotel_facility_name: hotelFacilityInput,
        hotel_facility_icon:
      ...
      setHotelFacilityData((prevData) => [...prevData, newHotelFacility]);
  ...
  const deleteFacilityType = async (id: string) => {
    try {
      await axios.delete(`${api}/hotel-facility/${id}`);
      setHotelFacilityData(hotelFacilityData.filter((type) => type.id !== id));
    } catch (error) {
      console.error("Error:", error);
    }
  };
  ...
  interface DataType {
    ...
    id: string;
    hotel_facility_name: string;
    hotel_facility_icon: string;
            ...
            <div className=" w-1/2 px-8">
              ...
              <div className="py-4 flex gap-3">
                ...
                <div className="flex flex-col w-full gap-2">
                  <Input
                    placeholder="Type Hotel Facility"
            ...
            <div className="w-1/2 px-8 border-l-[1px] border-stroke">
              <div className="w-full">
                ...
                <div id="scrollableDiv" className="h-[74vh] overflow-auto px-3">
                  ...
                  >
                    <List
                      dataSource={hotelFacilityData}
                      renderItem={(item) => (
                        <List.Item key={item.id}>
                          <List.Item.Meta
                            title={
                              <div className="flex gap-2">
                                <div>{item.hotel_facility_name}</div>
                                <span>-</span>
